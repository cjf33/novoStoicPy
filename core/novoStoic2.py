...
def novoStoic_rxn(model,radius,substrates,products,novel_mets,iterations,
    ...
    for things in met_result:
        ...
        #pdb.set_trace()
        for j in y_store_rules.keys():
            for jj in y_store_rules[j].keys():
                integer_cut_rules = y_store_rules[j][jj]
                for r in range(0,len(integer_cut_rules)):
                    #pdb.set_trace()
                    if integer_cut_rules[r] not in rules_index:
                        rules_index.append(integer_cut_rules[r])
                        rules.loc[:,integer_cut_rules[r]] = pd.Series(rules_store[integer_cut_rules[r]], index=rules.index) 
...
#        for j in y_store_rules.keys():
#            for jj in y_store_rules[j].keys():
#                integer_cut_rules = y_stor_rules[j][jj].tolist()
#                for r in range(0,len(integer_cut_rules)):
#                    if y_store_rules[j][jj][r] not in y_rule.keys():
#                        y_rule[y_store_rules[j][jj][r]] = ''.join('y_rule_'+y_store_rules[j][jj][r])#

#        for j in y_store_rxns.keys():
#        	for jj in y_store_rxns[j].keys():
#        		lp_prob += pulp.lpSum([y_rxn[r] for r in y_store_rxns[j][jj] ]) + \
#                    pulp.lpSum([y_rule[r] for r in y_store_rules[j][jj] ]) <= length,\
#                                 'intger_cult_' + str(counter)

        pulp_solver = pulp.solvers.CPLEX_PY()
        # lp_prob.writeLP('kegg_novostoic_isobutanol.lp')
    	y_store_rxns[things] = dict()
    	y_store_rules[things] = dict()
        for sol_num in range(0,iterations):
            integer_cut_rxns = []
            integer_cut_rules = []
            lp_prob.solve(pulp_solver)
            ...
            #        f.write('\n')
            #pdb.set_trace()
            y_store_rxns[things][sol_num] = integer_cut_rxns
            y_store_rules[things][sol_num] = integer_cut_rules
            #pdb.set_trace()
