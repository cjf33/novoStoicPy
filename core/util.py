...
def loadPrimes():
    f = open(os.path.join(data_dir,'primes.txt'))
    ret = []    
    for line in f:
        a = line.split()
        for i in a:
...
def processPrm(G,radius,product_list): 
    ...
    prdPrm = {v : primes[product_list.index(str(prdDict.get(v)))] for v in prdDict} 
                      
    nx.set_node_attributes(G, prdPrm, prmId)    
    return G
...
def processPrd(G,radius):    
    ...
    nx.set_node_attributes(G, prdList, prdId_nxt)
